...
def get_schemas_from_json(fpath):
    ...
    for db in data:
        ...
        for i, tabn in enumerate(table_names_original):
            ...
            cols = [
                str(col.lower()) for td, col in column_names_original
                if td == i
            ]
            schema[table] = cols
...
def parse_table_unit(toks, start_idx, tables_with_alias, schema):
    ...
    idx = start_idx
    len_ = len(toks)
    if toks[idx] not in tables_with_alias:
        key = toks[idx]
    else:
...
def parse_value(toks,
    ...
    elif "value" in toks[idx]:  # token is a string value
        val = toks[idx]
        idx += 1
    else:
        try:
            ...
            end_idx = idx
            while end_idx < len_ and toks[end_idx] != ',' and toks[
                    end_idx] != ')' and toks[end_idx] != 'and' and toks[
                        end_idx] not in CLAUSE_KEYWORDS and toks[
                            end_idx] not in JOIN_KEYWORDS:
                end_idx += 1

            idx, val = parse_col_unit(toks[start_idx:end_idx], 0,
                                      tables_with_alias, schema,
                                      default_tables)
            idx = end_idx
